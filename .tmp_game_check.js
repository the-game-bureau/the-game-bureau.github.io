    ...
    function showTextInput(stop, startIndex = 0) {
      ...
      _submitHandler = function() {
        ...
        if (anytimeMatch) {
          ...
          showMessages([...msgs, ...cr], () => showTextInput(stop, startIndex));
          return;
        }

        const answers  = triggerBubble
          ? (triggerText ? [triggerText] : [])
          : (reply.answers || []).map(a => String(a || '').trim()).filter(Boolean);
        const isAny    = !triggerBubble && (reply.type === 'any' || answers.length === 0);
        const correct  = triggerBubble
          ? answers.length > 0 && answers.some(a => norm(a) === norm(val))
          : (isAny || answers.some(a => norm(a) === norm(val)));
        ...
        const correctReplies   = triggerBubble
          ? (triggerBubble.replyCorrect ? [{ html: triggerBubble.replyCorrect }] : [])
          : (Array.isArray(reply.correct) && reply.correct.length ? reply.correct : []);
        const incorrectReplies = triggerBubble
          ? (triggerBubble.replyIncorrect ? [{ html: triggerBubble.replyIncorrect }] : [])
          : (Array.isArray(reply.incorrect) && reply.incorrect.length ? reply.incorrect : []);
        ...
        const toShow = [];
        if (correctReplies.length) toShow.push(pick(correctReplies));
        ...
        if (savedStep > 0) {
          for (let i = 0; i < savedStep; i++) {
            ...
            (stop.messages || []).filter(m => (m.html || m.text || '').trim()).forEach(m => {
              addBubble(m.html || m.text || '', !(m.fromPlayer || m.direction === 'fromPlayer'));
            });
          }
        }
      playStop(savedStep);
