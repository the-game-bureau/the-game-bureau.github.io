...
- Track lists live in `soundtracks.json`, keyed by `city_slug`.
...
| `soundtracks.json` | Runtime song data only: `city_slug` plus `songs[]` (`title`, `artist`, `spotifyId`, `blurb`, optional `explicit`). |
