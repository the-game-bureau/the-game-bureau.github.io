...
- **The list is appended to, not replaced** (changed 2026-08-04, so a second run in a day no longer wipes the first's picks). Each run reads `queue.json`, keeps every existing post as-is, and adds its five to the end, deduping against every `url`/`headline` already in the file. Post `id`s are stamped with the run's UTC time (`<YYYY-MM-DD>-<HHMM-UTC>-N`) so same-day runs never collide — the id is only ever an opaque key (the admin trashes on `post.id || post.url`), never parsed for its date. Consequence: the queue now **accumulates** — POST/TRASH are client-side and never write back, so nothing prunes it server-side. If it needs trimming, that's a deliberate future change (a cap on `posts`, or an age cutoff), not a regression.
