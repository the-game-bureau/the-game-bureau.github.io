...
### THE WAYPOINT FINDER — reviewing places, not ordering them (2026-08-18)

**[mc/waypoint-finder.html](mc/waypoint-finder.html)**, a Daily Chore on `/mc/` (**FIND WAYPOINTS**) and listed in **Game Elements** beside the Path Builder. The two are halves of one job: this decides whether a place belongs in the catalogue at all, the Path Builder puts the kept ones in an order.

- **ONE TOGGLE, TWO STATES** (2026-08-18). `public.waypoints.archived` is the whole status and always was: boolean, NOT NULL, no third value. **SHELVED = true, LIVE = false.** It was three buttons of which two were always the same pair with one greyed, which is a switch drawn as two buttons; a switch also says what the row IS, not only what pressing it does.
  - **EVERYTHING ARRIVES SHELVED** since [2026081803_waypoints_arrive_shelved.sql](mc/supabase/migrations/2026081803_waypoints_arrive_shelved.sql) flipped the column's default to `true` — a place from Waypoint Bot, one found online, one typed by hand in either room. One arrival state is a thing you can hold in your head; two is a rule you have to look up. Same call the Tape Room made, where a manually added tape arrives shelved too. **Existing rows were not touched**: a default applies to future inserts only, and re-shelving 300 places that have been live for weeks, several of them on paths, would empty the library to make a point.
  - **DELETE IS GONE.** Shelving is the stronger answer: the row stays as a do-not-rescrape tombstone the importers match on, so a **deleted** place comes back on the next sweep and a shelved one does not. A genuine delete is a `DELETE` in the SQL editor, where the confirmation is that you typed it.
- **HISTORICAL, superseded by the toggle above — three verbs: LIVE · SHELVE · DELETE**, and only the last is irreversible, so only the last asks. The confirm names the place rather than saying "this item", and points at the thing you probably meant: **a shelved row still stops an importer re-adding the place**, and a deleted one lets the next run put it straight back. `archived` on a waypoint is a **do-not-rescrape tombstone**, which is exactly why Shelve keeps the row.
- **`archived` is the only state** — LIVE is `false`, SHELVED is `true` — the same two-state model as a soundtrack, and for the same reason: it is the flag every other reader already filters on.
- **NEW IS A LENGTH OF TIME, NOT A STATE** (`NEW_FOR_HOURS = 72`). This is the Tape Room's lesson taken up front rather than paid for twice: a state-based queue accumulates until somebody clears it by hand and had reached 246 candidates, while a time-based one empties itself.
- **`public.waypoints.created_at` was added for it** — [2026081802_waypoints_created_at.sql](mc/supabase/migrations/2026081802_waypoints_created_at.sql). **Nullable, and existing rows stay null**: backfilling `now()` would declare 280 places new on the morning it runs, which is the one thing the column exists to answer correctly, and backfilling a made-up date would put a guess in a record meant to be a fact. Null means "arrived before we started counting", the way a null `lat`/`lon` means "not located yet". The page reads a null as **not new**, so it works before the migration is applied.
- **The counts on the filter strip and the list come from ONE predicate.** `countFor` borrows `state.tab` for the length of the call rather than reimplementing the test; two implementations of "what is in this tab" is how a strip ends up disagreeing with the list beneath it.
- **A write that RLS refuses is caught.** PostgREST answers **200 with an empty array** when a policy blocks a PATCH — no error, no rows, nothing changed — so both writes send `Prefer: return=representation` and treat an empty body as a failure. Without it the card repaints as though it saved and quietly reverts on the next load.
- **The tags say what is MISSING**, not what is present: `no source` and `not located`. A source url is on almost every row and says nothing about those; its absence is what is worth acting on, since it is what keeps a claim checkable.
- **IT WEARS THE SOCIALIZER'S QUEUE, PART FOR PART** (2026-08-18). It had been the shared `.room-head` over a plain `.panel` of compact rows with pill filters — a table with a switch on the end. It is now the Socializer's list: a tinted **`.queue-panel`** with a folder-tab legend, a **`.queue-bar`** carrying the segmented **`.queue-tabs`** strip on the left and the tally on the right, and a **`.queue-list`** of white **`.post`** cards with air between them. The CSS was copied from [mc/socials/index.html](mc/socials/index.html) rather than reconstructed, which is the standing rule for this vocabulary — **the Socializer is the reference and every other room is a copy**, so when one changes, change the others.
  - **The card is the Socializer's three bands**: a mono **kicker** across the top (`WPID 312 · MIAMI`, and how long ago it arrived), the place itself in the body with the name as the largest thing on it, and **the decision in its own band underneath**. The toggle used to sit in a gutter on the right, where four small controls read as row furniture; across the bottom it reads as the one thing you can do. Order is load-bearing: *what the place is, then what you can do about it*.
  - **THE ARRIVAL LINE IS OMITTED WHEN THERE IS NO DATE, not filled with a non-answer.** Until [2026081802](mc/supabase/migrations/2026081802_waypoints_created_at.sql) lands no row has a `created_at`, so "arrived before we counted" printed on all 312 cards — the loudest thing in the room, saying nothing about any of them. Same rule the Tape Room applies to `added N ago`.
  - **Pills became a segmented track** for the reason the Tape Room's did: four buttons in a row read as four independent controls, one inset track with the active segment raised reads as four views of one list. The counts are still the whole reason it is not a dropdown.
  - **The legend carries the CITY** (`WAYPOINTS: MIAMI`) — the one thing narrowing the list that the tab strip cannot show — and the tally carries the count and nothing else. The old `.panel-head` said both plus the state, which the strip beneath it was already saying.
...
- **PATH CLONE SITS SECOND, PATH BOT LAST** (2026-08-18). The ADD bar's path half reads Path Manual · Path Clone · Path Bot: the two working ways to make a path are next to each other, and the one that is not wired to anything sits at the end, where an unwired control belongs.
...
- **SHELVED WAYPOINTS ARE SHOWN, IN A DIFFERENT GREY** (2026-08-18). They used to be filtered out of `state.waypoints` on load, so a shelved place vanished from this room entirely — and once arrivals started landing shelved that would have hidden every new waypoint from the page where paths are built. **The two greys mean opposite things and must stay distinguishable**: used-grey is *done, it is already on the walk*, so the row dims; shelved-grey is *not approved, decide in the Waypoint Finder first*, so the row keeps its contrast, goes colourless, takes a hairline down its left edge and says `shelved`. Dimming both the same way would tell you a waypoint nobody has looked at has already been handled.
...
- **Remove takes a stop off the path and keeps the waypoint.** That is the entire reason these are two tables.
