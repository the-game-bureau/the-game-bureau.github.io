...
- **`tour_id` is readable, not a sequence** — `new-orleans-loop-20260808-134216` — so a row's tour is legible in the table editor without a join. It is city + shape + **UTC timestamp to the second**; seconds rather than minutes because to the minute a re-paste of the same city and shape inside one minute produces the *same* id and silently merges into the first tour, giving one twenty-stop walk. `now()` is the statement timestamp, so every row of one call still shares one id.
- **It is the "route id" this table was going to need a `routes` table for**, and it is deliberately not called `route_id`. **Route** is taken: in the canonical hierarchy a Game contains Routes and a Route's real ordering is `public.stops.ord`. A set of waypoints sharing a `tour_id` is a *catalogue* object — a sellable walk — not a game's route, and naming it `route_id` would put two different things one word apart. Same reason `walk_order` is advisory. (`route_color` is a third unrelated use of the word: the engine's rotation slot.)
...
### The Waypoints card — read-only until you press Edit (2026-08-08)

- **Cards are LOCKED by default.** Every field used to be a live input, so a stray click while scrolling ten cards could alter a description and the only sign was the Unsaved badge. **Edit** unlocks one card; **Save** locks it again, because the edit is finished and leaving a trail of unlocked cards behind you defeats the lock. The lock is applied in **one pass over the finished card** (`querySelectorAll('input, textarea')` → `readOnly`, plus `disabled` on the inline helper buttons), not threaded through the six field renderers — so a field added later is locked by default rather than editable because somebody forgot to pass a flag.
- **Duplicate** makes a new row from an existing one, dropping exactly three things: `wpid` (the trigger assigns it), the three `tour_*` columns and `walk_order`. **A copy is a LOOSE waypoint** — inheriting the tour would give that walk two stop 4s, and order is the one thing a tour cannot survive losing. `lat`/`lon` **do** come across (same place, same point, and it saves a 1-per-second geocode) and `archived` never does (a copy is new work, not something already ruled out). The name gains ` (copy)`, which is also what keeps the importers' name+city check meaningful. It skips the are-you-sure-this-is-a-duplicate warning, since resembling an existing waypoint is the entire request.
- **Every column is on the card**, in a dashed strip under Source URL: point, order, tour id/title/shape, AI, archived. Read-only, because each has exactly one writer — `lat`/`lon` from `persistPoint`, `walk_order` from Suggest order, the tour columns from `tgb_import_walking_tour`, `wpid` from a trigger, `archived` from its button. **An absent column reads "no column" in red, not blank**: null is a gap the page can fill, missing means a migration has not run. WPID is deliberately *not* in the strip — the card head already shows it.
- **The tour id on a card is a FILTER.** Clicking it shows that walk and nothing else, switches the city to the tour's own, and puts `?tour=` in the URL — a finished walk is the thing most worth linking to, so `?tour=` outranks `?city=`. The page title becomes the tour's name, shape and stop count with a **Show whole city** button, because a view holding ten of a city's forty waypoints with no explanation reads as missing data. Picking a city clears the tour; while a tour is active the city filter is **skipped entirely** rather than intersected, since a tour is already a subset of one city.
- **Carrying `walk_order` on the row fixed the sort, not just the display.** `compareRows` had always fallen back to `row.walk_order` when no suggestion was on screen — but `normalizeRow` dropped the column, so that branch could never fire and a saved order never ordered anything. Both halves are needed: the column in `normalizeRow`, the fallback in `compareRows`.
- **The city is in the URL** (`?city=new-orleans`, `?city=all`). Without it every refresh dumped you back in New Orleans no matter which city you were editing, and one city could not be linked to at all. `?city=` **outranks** the New Orleans default; an explicit `?city=all` is a real choice and survives a refresh, which is why an absent parameter and `all` are distinguished rather than both falling through. A stale or mistyped city opens All rather than an empty list. `replaceState`, not `pushState` — the city bar is a filter, not navigation, and sixty history entries would make Back useless for leaving the page.
- **There is no column list anywhere in the page.** Every read is `select=*`: naming a column PostgREST does not have 400s the *entire* page load, and this table gained six columns in three weeks. The old `WAYPOINT_COLUMNS` constant was deleted 2026-08-08 — it had not been used for anything except a comment explaining why it was not used.

### Geocoding a bare street line puts the pin on another continent

The first full backfill run put **36 of 274 points on the wrong continent** — Boston stops in London, a Baltimore synagogue in Melbourne, an Atlanta office in Sydney. The cause is worth writing down because the failing code looked obviously correct.

`waypoints.address` is **two different things**, and they need opposite handling:

| shape | example | how to query it |
|---|---|---|
| complete address | `200 E Colfax Ave, Denver, CO 80203` | **as it stands** — appending city/state/zip produces a string Nominatim cannot resolve |
| bare street line | `800 Ocean Dr` | **never alone** — that street exists in every English-speaking country and free-text search picks one |

The script tried the address as-is *first* for both. That is right for the former and catastrophic for the latter. **The comma decides**, and there are now two defences rather than one, because either alone still fails: the query always carries the city when the address has no comma, **and** every candidate point is measured against its own city (geocoded once, cached) and rejected past **75 km**. That bound is loose on purpose — it has to clear a stadium in Foxborough being 40 km from downtown Boston while still rejecting a different continent by three orders of magnitude. No threshold separates "wrong side of the county" from "right", and pretending otherwise throws away good points.

It also **retries** (three tries, backing off) and isolates a failure to its own row: an unhandled transient network error threw away 100 rows of finished work mid-run. A network error is not an answer about an address — only an empty result is.

`WAYPOINTS_TABLE_DOC` now tells the models the same thing: **a street name with no number is worse than null**, because null means "not located yet" and a bare street name means "located, wrongly, in another city".

### A stale PostgREST schema cache looks exactly like an unapplied migration

Hit 2026-08-08 and worth recognising, because the wrong diagnosis is very convincing. `select=tour_id` returned **400** and `select=*` returned only the nine original columns, while the columns demonstrably existed in `information_schema` and `anon` held SELECT on every one of them. **PostgREST caches the schema**; until it reloads, a column that exists is a column it will reject. The page's own probes (`hasLatLonColumns`, `hasTourColumns`, …) then correctly conclude the migration has not run, and quietly degrade — which is the right behaviour and also a perfect disguise.

The fix is one statement: `notify pgrst, 'reload schema';`. **Check this before concluding a migration was never applied.**

### `supabase db query --linked` is full postgres access, and it does work here

`mc/supabase/.temp/linked-project.json` pins `qmaafbncpzrdmqapkkgr` ("Games") and the CLI connects as `postgres` — so DDL, data fixes and `notify pgrst` are all available from `mc/` without a service key. **Check the ref before running anything**: `supabase projects list` shows a *different* project (`jdyjyzakcfmvwdfgzzyz`), which is the trap the folder-consolidation note already warns about, and the row counts differ enough to notice too late. A read through the **publishable key** shows fewer rows and fewer columns than the truth — that is RLS and the schema cache, not the real table.
...
- **The editor is [mc/stops.html](mc/stops.html)** ("Stop Builder"). **Third address:** it began at `mc/stops/stops.html`, moved to `games/admin/stops.html` on 2026-07-31 "to sit with the other game tools", and came back to `mc/` on 2026-08-07 when those tools moved — it now sits beside [mc/profiles.html](mc/profiles.html) and [mc/builder.html](mc/builder.html), the other two game editors. **`/mc/stops.html` and `/mc/stops/` are different live things**: the folder still holds `nightly.json`, which the scout routine commits at that path and the Waypoints page reads as `/mc/stops/nightly.json`. Its other half, [mc/data/waypoints.html](mc/data/waypoints.html) ("**Waypoints**" — called "Waypoint Finder" until 2026-08-07), moved out of `mc/stops/` on 2026-07-31 to sit with the other data catalogs, so the two link to each other across folders by absolute path. The old route builder `mc/mapper.html` was archived on 2026-07-30.
