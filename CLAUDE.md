...
- **Our caption travels with every post, on every platform** — writing one is pointless if the outlet's headline is what goes out. How far it gets differs because the platforms do: **X** opens its composer with caption *and* link in the box; **Facebook** gets the link plus the caption in `quote`, but honours `quote` inconsistently, so the caption goes to the clipboard as well and an empty box is one paste away rather than a retype (that is why `PLATFORM_PREFILLS.facebook` is deliberately `false` even though we send `quote`); **Instagram** and **YouTube** have no post-by-URL at all, so it is clipboard-and-paste. `PLATFORM_PREFILLS[key] === true` means "the composer is reliably prefilled, don't touch the clipboard".
