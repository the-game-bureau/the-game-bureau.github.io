...
- **THE CANDIDATES POPUP IS DELETED, AND `nightly.json` NOW HAS NO READER** (2026-08-18). The **WAYPOINT BOT** button in the Path Builder's ADD bar is a plain `<a>` to the routine at claude.ai and does nothing else. It used to open a dialog listing what the run had committed, with an **Open** per candidate that loaded it into the waypoint editor as an unsaved draft — that was the review step this file describes above, and it is gone from the UI. `openNightlyDialog`, `closeNightlyDialog`, `loadNightlyRun`, `loadNightlyPicks`, `renderNightlyStop`, `nightlyRelative`, `NIGHTLY_FILE`, `NIGHTLY_RUN_FEED` and `NIGHTLY_STALE_HOURS` all went with it.
