...
> **"TAPE ROOM" means [mc/soundtracks/admin/index.html](mc/soundtracks/admin/index.html).** Nothing else. It is the room's name on screen (the `<h1>`), it is what to call it in conversation, and an instruction naming it — *"add a button to the TAPE ROOM"*, *"the TAPE ROOM is showing the wrong counts"* — is an instruction about that one file, with no other page to check first. Never call it "the soundtracks admin" in new copy, comments or commit messages.
>
> | | |
> |---|---|
> | **file** | `mc/soundtracks/admin/index.html` — one self-contained page: markup, CSS and script in the one file, plus the AI prompt in a `<textarea>` |
> | **live** | <https://thegamebureau.com/mc/soundtracks/admin/> — public HTML on GitHub Pages, gated by the admin sign-in |
> | **local** | <http://127.0.0.1:5500/mc/soundtracks/admin/> under Live Server |
...
>
> **`/mc/soundtracks/admin/` and `/soundtracks/` are different things.** The second is the public cassette page a visitor sees, and only LIVE tracks appear on it.
...
- **The routine also audits, and reports to `public.soundtrack_issues`** (added 2026-07-30, [mc/supabase/migrations/2026073002_soundtrack_issues.sql](mc/supabase/migrations/2026073002_soundtrack_issues.sql)). Four kinds — `spotify` / `spelling` / `relevance` / `facts` — at three severities, shown in the Tape Room's **Issues** view plus a ⚠ chip on the affected track row. Scope per run is the two tapes it just wrote plus the **3 least-recently-audited**, ordered by the new `soundtracks.last_audit_at` (null first), so the catalogue is swept every couple of weeks at flat cost. **The agent reports and never edits** — same human-in-the-loop split as everything else here.
  ...
  - **THE ISSUES VIEW IS ONE FLAT LIST, ALWAYS OPEN** (2026-08-14). It was collapsible blocks per tape, which is the right shape for a long list and the wrong one here: a run files a handful, and a disclosure triangle in front of each meant the panel opened showing city names and no problems. **Every row names its own tape**, since flat there is no header to inherit it from — that costs one repeated word and buys a list you can read without pressing anything. There is no Expand all, no `<details>`, and no separate treatment for a whole-tape finding: it is a row like any other, wearing a `whole tape` chip instead of a track name.
  - **Two toggles, NEW and SKIPPED**, and the axis is whether the track is still in play. NEW is everything you can act on. **SKIPPED is findings against an already-skipped track, which were previously counted in a footnote and shown nowhere** — so un-skipping a track brought back findings nobody had ever seen. Only NEW feeds the room tab's badge; a finding against a skipped track is not work and must not colour it red. The kind filter that used to sit here (All / Spotify / Spelling / Relevance / Facts) is gone — the kind is a chip on every row, and four kinds across a handful of findings is not a haystack.
  - **Two buttons on every finding: Shelve <thing> · Skip issue.** View tape and View track were tried and removed the same day — with a shelve button on the row there is almost nothing left to go and look at, and a dialog you rarely open is a dialog whose contents go stale. **The whole popup went with them** (`openDataDialog`, `focusTapeForIssue`, `focusTrackForIssue`, the `.data-body` styles, and the `onDone` argument both editors had grown to close it) rather than being left unreachable — a viewer nothing can open is the "flag that no longer feeds anything" trap this file records elsewhere.
  - **Shelve track shelves it AND skips the finding in one press** — for most findings (a wrong Spotify ID, a bad fact, no real tie to the city) the track should not be on the tape at all, so that is one press rather than View track, judge, come back, Skip issue. Quiet, like Shelve tape, since `setTrackStatus` already reports the outcome. **Without the second half the finding would not vanish, it would merely move to the SKIPPED toggle**, which is not what a decision taken here should feel like.
  - ⚠️ **KNOWN WORDING SPLIT, left deliberately rather than guessed at.** The button says *Shelve track* while the same action is *Skip* on the track row, the state chip is *Skipped* and the notice reads *skipped*. It follows the instruction literally and mirrors *Shelve tape*, but it is one action under two names. The coherent end state is probably **KEEP / SHELVE / REVIEW** for a track (so SHELVE always means "off the site", at either scope, and SKIP is only ever about an issue) — that is a fourth vocabulary sweep and has not been done. Don't half-fix it: move every track-state word together or leave it.
  - **A finding on the SKIPPED toggle gets ONE button, RENEW.** The track is already off `/soundtracks/` and can never be re-picked for that city, so there is nothing to fix, nothing to look at and nothing to clear — skipping is the strongest answer available and it has already been given. The only question left is whether that was right. Renew puts the track back to **REVIEW, not LIVE**: un-skipping is a change of mind about the judgement, not a judgement of its own, and going straight live would decide the thing you just reopened. The finding stops being suppressed and reappears under NEW in the same press. Keyed off `isSuppressedIssue(issue)`, not off which toggle is showing — otherwise the buttons would be a fact about where you are standing rather than about the row.
  - **A TAPE finding and a TRACK finding get different buttons**, because they are answered differently. A track finding offers **View tape · View track · Skip issue**; one naming a track that has since been deleted or moved drops View track. A tape finding offers **Shelve tape · Skip issue** and *no* viewer: every tape-level finding says something about the tape as a whole — short of 15, over 15, a city that should not be published — and the answer is nearly always to take it off the site or accept it. An editor whose only fields are the spine phrase and which side it reads on answered neither. The button mirrors the tape list's toggle, so an already-shelved tape offers **Restore tape**, and it disables itself when the finding names a city no tape row is loaded for.
    - **Shelve tape also skips the finding, in the same press** — shelving answers it, and leaving it on the list would ask you to act on something you just acted on. Quietly (`resolveIssue(issue, 'fixed', true)`), because `setTapeArchived` has already put the outcome in the red pen and two notices for one press is one too many. **Only on success**: a shelve that failed has answered nothing.
    - **Restore tape does NOT skip it.** It is the opposite move — putting the tape back on the site does not address what the finding says, and is usually the thing the finding was complaining about. **The track's Skip button was dropped** from these rows in the same pass; skip a track from the track itself.
  - **Four chips per row, widest scope first: TAPE or TRACK · city · track name · issue type.** The first is the WORD, not the thing, and it is filled navy — the two shapes of row separate before you read either. A tape finding simply omits the track name; the old `whole tape` chip is redundant once the first pill says it.
  - **View tape and View track OPEN A DIALOG**, they do not send you to the other view. Switching views and scrolling lost your place in the findings and, on a long catalogue, dropped you somewhere you had to scroll back from. The dialog's body is filled by **the same `buildTapeEditor` / `buildTrackEditor` the Tapes & Tracks view builds** — a second editor is how the two drift, and it would be the copy that quietly stops accepting a field the other gained. Both grew an optional **`onDone`** argument, which is what closes the dialog on Cancel or a successful save/move/copy; inline in a row that is about to be re-rendered it is simply not passed. The body is emptied on **close**, not on open, or a half-typed edit survives to reappear on the next open as though it were saved. View tape also shows the tape's counts above its editor, since "is this tape short?" is usually the real question behind the press.
  - **`pendingFocusSongId` and `.is-flagged-focus` are gone** with that change — the Issues view used to set a pending id, switch view, auto-open the row's editor and scroll to it. Nothing reaches into `renderTrackRow` from outside any more.
  - **Skip issue still writes `status = 'fixed'`, not `dismissed`.** Only the label changed (2026-08-14, from *Clear issue*). The partial unique index blocks a re-report only while a finding is `open`, so a skipped finding comes straight back on the next audit if the problem is still there — that recurrence is the only check a fix landed. `dismissed` remains the status this page refuses to write.
  - **There is no tally line and no empty-state line.** "13 open findings across 7 tapes" restated what the toggle's count already said and what the list itself shows, and sat between you and the findings on every load; "The last audit found nothing to report." sat directly above a well already reading "Nothing flagged. The daily audit reports here when it finds something." The status channel is now only ever written for **sign-in state and write failures** — the two things the list genuinely cannot say on its own.
  - `resolveIssue` filters the **whole** loaded set, not `openIssues()`. Filtering the actionable subset discarded every skipped-track finding on each clear — invisible while they had nowhere to be seen, wrong now the Skipped toggle shows them.
...
- **TAPES & TRACKS and ISSUES are two VIEWS of one panel**, reached by folder tabs cut into its top edge. They were two stacked panels, so the catalogue always sat under a findings box that is empty most days — and the fix for *that* was an `.is-clear` rule collapsing the Issues panel to its title line, which made the page change shape depending on the data. **Both are gone.** The count on the tab is the signal the collapse used to be, and it goes red-pen when there is anything open. Tapes & Tracks is always the landing view whatever the data says: opening on Issues when findings happen to exist would mean the room greets you differently on different mornings.
  - The tabs are an ordinary element, not a `<legend>` — a legend holds one label and there are two views — so the panel is a `<div>`, not a `<fieldset>`. Each tab carries `margin-bottom: -1px` and draws its own bottom edge: the active one in the panel's fill so the two read as one sheet of card, an inactive one in the panel line so it stays behind. The `.command-bar` legends keep the separate `::before` arch.
  - **Edit on a finding now crosses views**, and must: `focusTrackForIssue` calls `showView('tracks')` *before* rendering, because scrolling to a row inside a hidden panel does nothing and the press reads as a dead button. It also **clears the status filter**, since a finding is routinely about a track the current filter hides — a Spotify problem on a LIVE track while you are looking at Review.
- **Segmented tabs replaced both filter `<select>`s** inside those views. Tapes & Tracks is REVIEW / KEPT / SKIPPED / ALL, each with a live count; Issues is NEW / SKIPPED (see above). **The counts are the whole reason**: a closed dropdown cannot say six tracks are waiting on you, which is the question the row exists to answer. ALL stays the default in both, so the view opens on the catalogue as it always has. REVIEW's badge goes red-pen the moment it holds anything.
...
Set 2026-08-15. One schedule, staggered three minutes apart so five cloud sessions do not provision at the same instant:
...
- **Edits are batched, not saved per card.** Changing a field marks the card dirty; **SAVE ALL CHANGES** in the header writes every dirty item at once, and a `beforeunload` guard catches you leaving with unsaved work. Same bargain the Route Builder's order makes.
