...
| **Socializer** | [mc/socials/index.html](mc/socials/index.html) | `socials` | TGB SOCIALIZER BOT |
...
| TGB WAYPOINT BOT | `trig_018FbHnaU5DqB4GesPfABV2d` | `8 8,20 * * *` |
...
>
> **IT LEFT `/mc/socials/` ON 2026-08-19 AND CAME BACK ON 2026-08-20.** For one day it was `mc/socializer.html`, named by a file rather than by a folder, which is the OPPOSITE of what every other room does (`/mc/gifts/`, `/mc/soundtracks/`). It is `mc/socials/index.html` again and matches the convention.
>
> **THE ROUND TRIP COST NOTHING, WHICH IS LUCK RATHER THAN DESIGN.** `#edit=` links are the thing that mattered: TGB SOCIALIZER BOT has mailed five `https://thegamebureau.com/mc/socials/#edit=<id>` links twice a day since August, every one of them broke on the 19th, and every one of them works again. **Had the move stood for a week, a week of mail would have been dead with no way to fix it** — GitHub Pages serves no 301, so a moved room takes its history with it.
>
> **WHAT THE TWO MOVES TAUGHT, worth more than the moves:**
>
> - **The asset paths only survived because they were made root-absolute on the way out.** The page's three `../../mc/…` script and stylesheet references became `/mc/…` on the 19th, so changing depth twice in two days cost nothing. A relative path would have broken both times.
> - **AN ESCAPED PATH HIDES FROM A SWEEP.** The admin nav's `match` regex is written `/^\/mc\/socializer\.html/`, and a search for `/mc/socializer.html` does not find it. It survived the first pass of the return move untouched, and a `match` out of step with its `href` does not error: it just quietly never lights the button. Grep for the escaped form too.
> - **The stored prompt is a third copy of the path** and moves with neither the file nor the repo. It is edited by hand on the trigger, both times.
>
> Every in-repo reference was repointed in the same commit, both times — the hub's Ancillary Things card in `mc/index.html`, the Socializer entry in `mc/js/admin-nav-menu.js` (there is no `match` regex for this room there, unlike the Tape Room's), the shared admin nav's FOLLOW button **and its `match`**, the page's own `tgb-agent-context` block and PROMPT text, both Edge Functions' comments, `PROMPTS.md`, and the palette-source comments in `mc/gifts/index.html` and `mc/pathbuilder.html`.
>
...
[mc/socials/index.html](mc/socials/index.html) — **the Socializer** — shows social post candidates, found by a **scheduled Claude Code cloud agent** (**"TGB SOCIALIZER BOT"**, `trig_01KDYndJhZ9ymgUgX5Xx6LsL`, cron `14 8,20 * * *` UTC — **twice a day**, 3 AM and 3 PM Central (2 o'clock in winter; see the schedule note above, nobody adjusts)).
...
- **THE PAGE CANNOT DO IT**, which is the whole reason it is a function: `/mc/socials/` is static HTML on GitHub Pages and CORS forbids reading another origin's markup.
...
### TGB PATH BOT — the sixth routine, and the first one that writes a PATH (2026-08-18)
...
**What it is for, and how it differs from the two routines it sits between.** TGB WAYPOINT BOT sweeps Wikipedia for individual PLACES in one city and commits a JSON file. TGB ANCHOR EVENTS DESIGNS a six-stop walk from `buildTourPlacesWaypointPrompt` and commits SQL for a human to run. TGB PATH BOT does neither: it finds a walking tour **somebody has already published** — a historical society, a preservation trust, the NPS, a visitor bureau — and transcribes that walk, in the order its author put it in. The argument is the same one that produced *With AI (tour places)*: somebody already decided the place was worth walking to and wrote down what order to see it in, and that judgement is expensive to recreate and free to read.
...
- **The TGB PATH BOT button in the Path Builder's ADD bar is now a real door**, an `<a>` to the routine, the same as TGB WAYPOINT BOT beside it. It read "not wired yet" until this landed.
...
**`/follow/` EXISTED FOR ONE DAY AND IS DELETED.** It was a public page listing the five accounts; five links did not earn a page load, a scroll and a way back. **FOLLOW is now a control that opens a Linktree-shaped menu** of the accounts, and it lives in exactly ONE place: the public nav.

- **`shell/site-nav.js` OWNS THE LIST**, exported as `TgbNav.socials()` and as `TgbNav.followPopup(el)`. **THE FOOTER HAS NOTHING TO DO WITH FOLLOW ANY MORE.** It has carried three different answers in two days: a column of five icon links, then a link to `/follow/`, then a button opening this same menu. All three are gone and The Site column is back to its four sections. `followPopup` is still exported and is still the way to add a second trigger if one is ever wanted; nothing calls it but the nav.
...
- **THE PANEL FLIPS UPWARD when there is more room above than below**, decided at open time from `getBoundingClientRect`. Nothing needs that today, now the footer trigger is gone and the nav sits at the top of the page; it is kept because the check costs one measurement and it is what makes `followPopup` safe to attach to a trigger anywhere.
