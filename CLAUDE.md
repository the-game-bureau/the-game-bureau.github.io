...
nothing they can act on. Catch it and say which file to run. The Waypoints room and
...
node -e "
const fs=require('fs');const s=fs.readFileSync('mc/waypoints/index.html','utf8');
...
2. **"A path consists of WAYPOINTS" vs "A Path contains an ordered list of
   ...
   that has not happened yet. **The Waypoints room builds waypoints in an order.
...
### THE DESTINATION IS A PIN CARRYING THE GAME'S OWN ICON (2026-08-30)

A dot says a place is here. **A pin with the game's emoji in it says WHICH
game**, which is the question a map of several cities has to answer at a glance.
The resting map is pins too, so "here is where we play" reads as a set of games
rather than a scatter of anonymous dots.

- **THE ICON FALLS BACK THROUGH TWO COLUMNS, because neither covers the table.**
  `default_emoji` on 17 of the 31 live rows, then `category_icon` on 22 -- which
  is a NAME (`football`, `music`) rather than a glyph, so it is the wider net and
  both are worth reading. **Six games carry neither and get a bare pin**, which
  is honest: an invented icon would say something about the game nobody decided.
- **THE TAIL IS A CSS TRIANGLE, NOT A ROTATED SQUARE.** The classic
  `border-radius: 50% 50% 50% 0` plus `rotate(-45deg)` teardrop changes the
  element's BOUNDING BOX, and Leaflet anchors on that box, so the tip would stop
  sitting on the coordinate it names. `iconAnchor: [19, 46]` puts the point on
  the place; anchored at the centre it would float half a pin north, which at
  street zoom is a different corner.
- **THE PIN BODY TAKES `currentColor`**, so the away club's colour reaches it
  with no second rule and no second asset, and the emoji keeps its own colours on
  top of it.
- **THE AWAY ORIGIN STAYS A SMALL DOT.** It is where they came from, not a place
  anybody stands, and two pins would read as two destinations.
- **THE LABEL AND THE GLYPH ARE ESCAPED.** They are row values going into a
  `divIcon` html string. A game named with an ampersand is ordinary; one carrying
  a tag is not, and neither should reach the DOM as markup.

### AND THE HEADER (2026-08-30)

- **THE NAV LINKS STAY RIGHT AT EVERY WIDTH ABOVE THE PHONE BREAKPOINT.**
  `site-shell.css` sends them full width and left under 980px, which is right for
  a page whose content is one wide column and wrong here: this page is a rail and
  a map the whole way down, so the header reads brand-left / links-right. Below
  860px the shared behaviour is right again and is restored explicitly.
- **`The Cure for the Common Walking Tour` IS `nowrap`.** It is a 35-character
  line with nowhere useful to break, and wrapped it pushes the nav taller and the
  map shorter. **It may wrap again under 860px**, because at 390px one line is
  not on offer and `nowrap` would simply overflow the page.

### LOSING A FILE TO A FAILED WRITE, TWICE IN ONE HOUR

**`io.open(path, 'w')` TRUNCATES BEFORE IT WRITES.** A script that throws
mid-write leaves an EMPTY FILE, not the original. `gamestest.js` was lost that
way and rebuilt; **CLAUDE.md was emptied the same way and the commit captured
it, 8,294 lines deleted**, recovered with `git show HEAD~1:CLAUDE.md`.

**WHAT THREW, BOTH TIMES, WAS THE ESCAPING SCAR.** A backslash-u escape written
into a quoted heredoc reaches Python having lost one level of backslash, which
Python decodes as a LONE SURROGATE, which utf-8 refuses to encode. This file has
recorded that scar eleven times for NUL bytes, newlines and word boundaries;
this is the twelfth and the first that destroyed a file.

**TWO HABITS, and the second is the one that actually saves you:**

1. **Write tooling with no backslash escapes at all.** `chr(92)` for a
   backslash, the literal character for anything printable, `split()` instead of
   a regex.
2. **BUILD THE WHOLE STRING FIRST, THEN OPEN FOR WRITING.** An edit script that
   opens the destination before it has finished computing what to put in it is
...
- **A ROOM JOINS THE FILE WHEN IT GETS A DOOR.** Waypoints and the Green Room have blurbs and no Ancillary Things card, so their sentence is written once, on the room, and has nothing to drift against. Add them the day they get a card, not before.
...
- **SIXTH ADDRESS.** `mc/stops/stops.html` → `games/admin/routes.html` → `mc/routes.html` → `mc/paths.html` → `mc/pathbuilder.html` → **`mc/waypoints/index.html`**. It now satisfies both standing rules at once, which is what none of the others did: **named after the room, AND a folder** like `/mc/gifts/`, `/mc/soundtracks/`, `/mc/socializer/` and `/mc/events/`. The Socializer took four addresses learning that; this is the same lesson arriving here.
- **THE NAME NOW COLLIDES WITH THE TABLE, AND THAT IS THE COST.** `mc/paths.html` was renamed away from `public.paths` on 2026-08-19 precisely because a room should not be named after a table it edits — and WAYPOINTS is named after `public.waypoints`, one of the **three** tables this room writes (`waypoints`, `paths`, `path_stops`). **The argument was heard and overruled**, the same way the Anchor Events room ended up at `/mc/events/`. What settles it in practice: the room's own doorways never stopped saying Waypoints — the Data Warehouse card and Daily Review's button both did, against a nav that said Path Builder, and this file already recorded that disagreement on 2026-08-17. **The room and its doors agree now.**
- **`public.paths`, `public.path_stops`, `paths.city` and `tour_id` ARE UNTOUCHED.** A Path is still a Path in the vocabulary, the hierarchy and the schema. This is a room's name and a filename, nothing more.
- **THE DEPTH CHANGE WAS FREE, AND THAT IS NOT LUCK.** Every asset reference on the page is root-absolute (`/mc/js/…`, `/mc/assets/…`), so moving from `mc/` into `mc/waypoints/` cost nothing. A single `../` would have broken all eleven. **This is the third page whose move has been cheap for exactly that reason** — write new cross-folder links root-absolute.
- **`?tour=` LINKS STILL WORK**, because the query contract is on the page and not on its filename — but a stored link that names the old file is dead like any other. `/mc/waypoints/?tour=<id>` is the shape now.
- **NOTHING CARRIED AN ESCAPED FORM OF THE OLD PATH**, checked rather than assumed. The admin nav's only `match` is `/^\/mc\//`, which covers the new address, and no regex anywhere named this page. That is the trap the Socializer sprang twice: a `match` written `\/mc\/socials\/` does not turn up in a search for `/mc/socials/`, and a `match` out of step with its `href` does not error, it just quietly never lights the button.
- **Repointed in the same commit:** the nav entry in `mc/js/admin-nav-menu.js` (href, label and its `action` line), the Data Warehouse card in `mc/data/index.html`, all three links in `mc/review/index.html`, Stop Builder's new-waypoint button in `mc/_stops.html`, the twelve provenance comments in `mc/assets/waypoint-editor.js`, the headers of `waypoint-geo.js` and `waypoint-prompts.js`, the chrome comments in `mc/events/index.html`, `mc/soundtracks/index.html` and `mc/teams/leagues.htm`, the note in `mc/js/room-blurbs.js`, step 5 of `path-bot.prompt.md`, and this file.
- **TGB PATH BOT's STORED PROMPT NEEDS A HAND EDIT AND IS THE ONE THING NOT DONE HERE.** It says *the Path Builder* and **never names a file**, so nothing is broken and nothing 404s — the routine writes through `tgb_pull_walking_tours` and does not open the page. The staged copy at [path-bot.prompt.md](mc/_dev/prompt-tools/path-bot.prompt.md) is updated; the trigger holds its own copy, which is the by-hand sync every routine prompt here needs. **TGB ANCHOR EVENTS needed no change**: it reads its spec out of `mc/assets/waypoint-prompts.js` and names no room.
- **MIGRATION COMMENTS WERE DELIBERATELY LEFT ALONE.** Six of them say *the Path Builder*; they are dated records of what the room was called on the day, and rewriting them would be editing history to match a name that arrived later. Same rule as the `apply by hand` headers.
- **AND THE PROSE THAT DESCRIBES THE MOVES WAS RE-READ AFTERWARDS**, which this file has had to learn twice: **a blind find-and-replace rewrites the HISTORY as well as the paths**. Every dated sentence below still says Path Builder, correctly, because that is what the room was called on the date it names.

**THE PAGE WAS `mc/pathbuilder.html` FROM 2026-08-19 TO 2026-08-28**, renamed from `mc/paths.html`. **A hard break with no redirect**, as always here, so `/mc/paths.html` now 404s. The point is that the file is named after the ROOM rather than after the table it edits: the room is called the Path Builder everywhere it is spoken about, and `paths.html` read as a listing of `public.paths` — which it is not, it is the editor for three tables. **`public.paths`, `public.path_stops` and `paths.city` are untouched**; this is a filename, not a rename of the concept, and the Route-to-Path rename below is the one that moved the vocabulary.
...
Migration: [2026080804_paths_and_path_stops.sql](mc/supabase/migrations/2026080804_paths_and_path_stops.sql). Editor: **[mc/waypoints/index.html](mc/waypoints/index.html) — "Waypoints"**, under **GAME ELEMENTS** in the nav (moved there from Game Builder on 2026-08-17: a Path is a reusable part a game is assembled from, like a Guide or a City, not one of the tools that does the assembling).
...
- **[mc/assets/waypoint-editor.js](mc/assets/waypoint-editor.js)**, the whole waypoint editor as a module: `window.TgbWaypointEditor`. It came out of `mc/pathbuilder.html` (about 550 lines, plus its dialogs and its CSS) when the Finder needed the same editor and had grown a five-field form of its own instead. That is the copy-and-drift this repo has already lost to twice, with the Plus Code codec and the waypoints import helper, and the reason `waypoint-geo.js` and `waypoint-prompts.js` exist.
  - **It has one caller again**, and that is fine: the file is a clean seam, the room's page is 550 lines lighter, and re-inlining working code to save a script tag is churn. **Don't re-inline it.**
  ...
  - **`probe()` MUST BE RE-RUN ONCE THE ROWS ARE LOADED.** `probeWaypointColumns` answers "no such column" for everything when handed an empty array, and `wpPayload` gates `lat`/`lon` and `source_url` on that answer. A host that mounts before it loads therefore writes the zip and **silently drops the coordinates and the source**, which is the worst kind of half-success because nothing reports it. The Waypoints room probes inside `loadAll`; Partners re-probes at the end of its own load.
...
- **Four call sites**: the Waypoints room's library rows, its path rows and its map popup, and the Partners card. Four hand-written anchors is how three end up missing `rel="noopener"` and the fourth swallows a drag.
...
- **`tgb_import_walking_tour` is untouched** and stays `SECURITY INVOKER`: one tour, run by a human under their own session from the Waypoints room's paste box. Two functions, same three tables, different callers.
- **The TGB PATH BOT button in the Waypoints room's ADD bar is a real door**, an `<a>` to the routine. It read "not wired yet" until 2026-08-18, and it sat beside a TGB WAYPOINT BOT door until 2026-08-20; **it is the only bot in that bar now**.
- **ZIP AND COORDINATES ARE PART OF ITS JOB AND HAVE THEIR OWN STEP IN THE PROMPT**, because they are the half most easily skipped and the cost lands on somebody else: an unlocated waypoint cannot be drawn on the Waypoints room's map, and geocoding it later costs a human 1.1 seconds a pin at Nominatim's one-request-a-second policy. The routine has the address in front of it already, so it is the cheapest place to pay that. The prompt carries the policy (sequential, 1.1s apart, identifying User-Agent, never parallelised), the accept test (the reply's `display_name` must contain the city asked for), and the standing rule that **a point is never invented or approximated** -- not from the city centre, not from a neighbouring stop, and never an intersection collapsed onto one of its streets, whose centroid can be a mile from the corner. A stop that genuinely cannot be located (an intersection, a span of street, a floor, a moored ship) is filed without one and named in the report.
- **Everything it files lands LIVE**, in the [Waypoints](mc/waypoints/index.html)'s library, ready to put on a walk. It filed shelved for one day, while a review room existed; see the all-live section above for what went with that.

### With AI (more places NEAR one of ours) — the cluster pull (2026-08-20)

A sixth pull in the Waypoints room's AI dialog, `buildClusterWaypointPrompt(city, anchor, count, notes)`. **It is a pull, not a new button**: the room already has one dialog with a picker, and a second door to the same idea is how two prompts start drifting.
...
> **A PARTNER IS A WAYPOINT.** It is a bar, brewery or other public food-and-drink room that will host visiting supporters of a rival fandom, wanted as the **last stop of a GAME run the day before the ANCHOR EVENT**. It is a row in `public.waypoints` carrying a `partner_status`, edited in the **Waypoints** room, and filed by **TGB PATH BOT**.
>
> | | |
> |---|---|
> | **where** | [mc/waypoints/index.html](mc/waypoints/index.html) — the `PARTNERS n` filter in the library header, and the partner band in the waypoint editor |
...
- **Edits are batched, not saved per card.** Changing a field marks the card dirty; **SAVE ALL CHANGES** in the header writes every dirty item at once, and a `beforeunload` guard catches you leaving with unsaved work. Same bargain the Waypoints room's order makes.
...
>
> **The end state** matches the canonical hierarchy already written above: `path_stops` gains `challenge_id` and `end`, a game points at a `tour_id` instead of inheriting its city's list, the Waypoints room grows a challenge picker, and `public.stops` + `game_stops` + `_stops.html` all retire. That touches **both engines** — the paid product — so it is a deliberate piece of work, not a follow-on.
...
**IT WEARS THE SOCIALIZER'S CHROME as of the same day.** The fourth room to take that port, after the Tape Room, the Gift Shop and Waypoints. **When any of them changes, change them all**; the Socializer is the reference and these are copies.
