...
- **A place appears at most once per path** — that is the `(tour_id, wpid)` primary key. A loop *finishes near* its first stop, it does not list it again.
...
- **Two columns: the path, and the city's unused waypoints.** The whole job is moving rows between them, so they sit side by side.
