...
- **THE COPY BUTTON NAMES THE DESTINATION WHEN IT IS THE WHOLE JOB.** `Copy to clipboard for X`, `Copy to clipboard for YouTube`, and a plain `Copy to clipboard` on everything else. `byHandTarget()` answers only when the machine can reach **nothing** for that row: a story tagged X *and* Facebook still posts to Facebook, so it keeps the plain label rather than claiming to be for X while about to go somewhere else. It reads names from the bot's own tags rather than `WIRE_LABELS`, because YouTube is deliberately absent from that object and would otherwise be unnameable.
