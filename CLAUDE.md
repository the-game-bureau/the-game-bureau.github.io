...
[mc/socials/index.html](mc/socials/index.html) shows five stories the socials accounts might want to post, found by a **scheduled Claude Code cloud agent** (**"TGB SOCIAL BOT"**, `trig_01KDYndJhZ9ymgUgX5Xx6LsL`, cron `0 8,20 * * *` UTC — **twice a day**, 3 AM and 3 PM Central in summer, 2 AM and 2 PM in winter (same clock as the gift shop and soundtrack bots; it was briefly `0 3,15` on 2026-08-05, which is 10 PM / 10 AM Central — the UTC offset is five hours in summer, not zero)). It commits [mc/socials/socials.json](mc/socials/socials.json) to `main`; the page is a read-only render of that file.

**Moved to `/mc/` on 2026-08-04**: the admin was `socials/admin/index.html` and the data file `socials/queue.json`; they are now [mc/socials/index.html](mc/socials/index.html) and [mc/socials/socials.json](mc/socials/socials.json) (they briefly sat at `mc/socials.html` + `mc/socials.json` the same day), and **the whole `socials/` folder is gone** (it also held a `socials/index.htm`). Hard break, no redirect, same as every other move here — GitHub Pages cannot serve a 301. Three things had to move with them and will silently rot if a future move forgets any: the **routine's stored prompt** (it names the file it commits), the **last-run GitHub commits API `path=` query** in the page, and the page's **`QUEUE_FILE`** resolve. **`QUEUE_FILE` anchors on the `/mc/socials/` path segment, and must.** The page is served from a *directory*, so a bare `new URL('socials.json', …)` is wrong: `new URL` measures from the current directory, and a request without the trailing slash (`/mc/socials`) resolves one level too high to `/mc/socials.json` and 404s. Most servers redirect to add the slash — not all, and an IDE preview often doesn't. The segment anchor covers `/mc/socials`, `/mc/socials/`, `/mc/socials/index.html`, any host, and `file://`. This was briefly simplified to a plain relative resolve during the few hours the page sat at `mc/socials.html`, where it genuinely was safe; **don't re-simplify it now** — that only holds for a page that is a file, not a directory.

**Use the name TGB SOCIAL BOT everywhere** — page button, admin copy, docs, routine name. It was called "Daily socials scout" / "TGB Daily Social Post Suggestions" until 2026-08-04; both are retired. A second, disabled routine (`trig_01VGJLzRyxWuvgrP4hv8ZMM5`, cron `15 11 * * *`, last fired 2026-07-30) is a **dead duplicate** left over from that rename, renamed to "OBSOLETE — superseded by TGB SOCIAL BOT (safe to delete)". Delete it at [claude.ai/code/routines](https://claude.ai/code/routines); don't re-enable it or you'll get two queues a day.

- **The prompt opens with a HOW TO RUN block, and it is load-bearing** (added 2026-08-04). A cloud run has nobody watching it, so the failure mode is a run that stops to ask a question and burns its slot: it never commits, and the only symptom is a stale timestamp. The block forbids asking for confirmation, and gives a named recovery for each way a run realistically dies — dead link, empty search, too few good stories, rejected push. **Never soften "never ask" into "ask if unsure"**; unsure is exactly when it would stall.
- **The append rule is spelled out as a six-step counted procedure**, not a sentence, because "append, don't overwrite" was already in the prompt on 2026-08-04 when the risk was still real. The version that sticks makes the model count the posts before and after and abort to `git checkout --` if the total isn't old + new, and forbids handing Write a document it composed without reading the current file in the same run. The rebase instruction says to keep **both** sides' posts on conflict — a conflict here is two runs appending, and taking one side silently discards the other.
- **The routine's stored prompt must stay identical to the PROMPT button's textarea** in [mc/socials/index.html](mc/socials/index.html) (`#promptText`) — that textarea is the source of truth, and the routine is a copy of it. They silently diverged for a week in 2026-08 (the routine still had the overwrite-the-file version after the page moved to append), which is exactly the failure this rule exists to catch. Edit the page, then paste into the routine; same rule the Tape Room has.

- **Vocabulary, standardized 2026-08-04 — use these four words and no synonyms.** A **story** is the article out in the world, what the bot hunts. A **candidate** is a story filed here awaiting a decision — *not* a pick, a queue item, a link, or (as a noun) a post. A **caption** is the text we would publish; the JSON key stays `blurb` and the JS variable stays `blurb`, but **no user-facing string says "blurb"**. **Post** is the verb, and the button. Before this pass the page called one thing a pick, a post, a link, a queue item and a candidate in five different messages. `pick` survives only as a verb ("Pick at least one platform") and in internal names (`picker`, `picked[]`, `.platform-pick`).
- **The agent posts nothing and holds no account credentials.** It writes one file; a human clicks **Post** (opens the prefilled composers) or **Skip**. Don't ever wire this to a social API — the human-in-the-loop is the design, not a missing feature.
- **The list is appended to, not replaced** (changed 2026-08-04, so a second run in a day no longer wipes the first's picks). Each run reads `mc/socials/socials.json`, keeps every existing post as-is, and adds its five to the end, deduping against every `url`/`headline` already in the file. Post `id`s are stamped with the run's UTC time (`<YYYY-MM-DD>-<HHMM-UTC>-N`) so same-day runs never collide — the id is only ever an opaque key (the admin skips on `post.id || post.url`), never parsed for its date. Consequence: the queue now **accumulates** — POST/SKIP are client-side and never write back, so nothing prunes it server-side. If it needs trimming, that's a deliberate future change (a cap on `posts`, or an age cutoff), not a regression.
- **The row has exactly two buttons, Post and Skip.** A separate **Copy** button (blurb + link to the clipboard) was removed on 2026-08-04: Post already puts the caption on the clipboard for the platforms that need a paste, so Copy was a second path to the same thing and an invitation to post without opening the composer. `clipboardText()` / `copyToClipboard()` stay — Post calls them.
- **Visible verbs are Post and Skip; internal names stay `autopost*`/`autoPost` and `trash*`** — the same split as HIDE over the `archived` column. In particular **`localStorage['tgb_socials_trashed_v1']` keeps its key**: renaming it to match the button would orphan every dismissal already on someone's machine and resurrect posts they had cleared. Don't finish the rename into the storage key or the helpers.
- **A card is split into a preview and its ephemera** (2026-08-04). The top half (`.post-preview`) is the post *as it will look*: the caption first and largest with no quote-block chrome — chrome would read as citing it rather than publishing it — then `.post-card`, the unfurled link shaped like what X and Facebook actually render (domain, headline, source · date), which is also the only link target. The bottom half (`.post-ephemera`, tinted and divided) is everything that is ours and never leaves the page: topics, the internal `why`, the buttons, the platform picker. **Keep that split when adding anything** — if it would appear in the published post it belongs above the divider, otherwise below.
- **The platform "Post to" pills were removed from the card** (2026-08-04). They duplicated the picker and buried their most useful half — the agent's per-platform `why` — in a `title` tooltip nobody hovers. **Each platform's why now sits beside its tick under the Post button**, where you are actually choosing: the agent's reason when it tagged that account, otherwise the standing format note from `PLATFORM_FORMAT_NOTE` so no row is blank. The card's tag row is topics only.
- **Post takes several platforms but opens them ONE PER CLICK.** Tick any of the four accounts (the agent's tagged ones are pre-ticked) and the runner opens the next composer each time you press it, tracking `N of M`. **Don't "improve" this into a loop that opens them all at once** — a browser grants one popup per user gesture, so three of four tabs get eaten by the popup blocker *silently*, leaving you sure you posted everywhere when you didn't. The per-step clipboard write has the same cause: Instagram's caption must land on the clipboard as its tab opens, not three steps earlier. Still no credentials and still no publishing — the composer is prefilled and a human taps Post.
- **SKIP is client-side**, remembered in `localStorage['tgb_socials_trashed_v1']` keyed by the date-stamped post id, so a dismissal survives a reload without needing a write path.
- **POSTED is marked by finishing a Post run**, not by a separate button (added 2026-08-04). When the last ticked composer opens, the candidate moves to the POSTED tab and `localStorage['tgb_socials_posted_v1']` records `{ at, platforms }` — a map, not a list, so the card can show "POSTED TO X · INSTAGRAM — Aug 4, 3:12 PM". **The page cannot know whether you actually pressed Post on X**, so POSTED honestly means "you took it all the way through the composer", and **Move to Review** returns it for the time you bailed out. Precedence in the split is posted > skipped > review: what you did with a candidate outranks what you passed on.
- **Candidates render newest run first, oldest last** — but only the *runs* are reversed; each run keeps its own 1..5 order, since that order is the bot's and a plain `.reverse()` would show every run backwards. Runs are grouped by the id minus its trailing `-N`; an id that doesn't match becomes its own group, so nothing is ever dropped. **Don't sort on `published`** — that's when the outlet published the article, routinely days before the run that found it, so it would bury this morning's candidates under an older story. The file on disk stays append-order (oldest first); the reversal is render-only.
- **The panel is SOCIAL POST CANDIDATES with three tabs, REVIEW, POSTED and SKIPPED**, all rendered from the same `posts` array — the skip list is the only thing that decides which side a story falls on, so there is one render path and no second source of truth. The panel title dropped its `generated_at` date when the tabs landed (2026-08-04); freshness lives in the last-run status, not the heading. **The header subhead (`#pageStatus`) is a standing explanation of the page, not a counter** — it used to read "5 posts waiting — 2 skipped", which the tabs now say better. Transient states (an error, "sign in with an admin account") still borrow that line, and `setPageStatus('')` restores the intro line rather than blanking it, so the header is never empty.
- **Undo is per-row on the Skipped tab, never bulk.** Each skipped row carries one button, **Move to Review** (`removeTrash`), and that is the only write the tab makes. The Posted tab's button says the same thing and does the same thing (`removePosted`) — both decided tabs have exactly one way back, named after where it goes rather than what it undoes ("Unskip"/"Unpost" were two words for one action). The old "Restore N Skipped" button in the panel header was removed first — it could wipe a week of decisions in one click, with no confirmation. **Don't reintroduce a clear-all**; per-row is what makes a misclick recoverable without making a mass reset easy.
- **Post is copy-open-paste, uniformly, and passes nothing** (simplified 2026-08-05). Every platform: caption + blank line + URL to the clipboard, that platform's composer opens, you paste and post it there. **No query strings, no prefill, no exceptions.** The previous version handed X a prefilled `intent/tweet` URL and Facebook a `quote` param; Facebook ignores `quote` outright (their policy is that you may not put words in a user's mouth before posting), Instagram and YouTube have no post-by-URL at all, so exactly one of four worked and the behaviour differed on each. One rule you can trust beat three exceptions. Composers live in `PLATFORM_COMPOSERS`. If a platform ever gains a prefill worth having, add it as an explicit exception and say so in the picker — never silently.
- **Every Post step loads the clipboard with caption + blank line + URL** — all four platforms, including the ones whose composer is prefilled. Not conditional on purpose: a prefill can silently fail (Facebook honours `quote` when it feels like it), and you often want the caption to hand regardless. Identical payload everywhere, so what you get never depends on which platform the run ended on. An Instagram-only variant that dropped the URL was tried and removed the same day — a predictable clipboard is worth more than saving one unclickable line. Deliberately never the headline (that's the outlet's words) and never the `why` field, which is an internal note for the admin page.
- Last-run status reads the **GitHub commits API** for `mc/socials/socials.json`: a run that errored pushes nothing, so a stale timestamp is the failure signal. (The soundtrack admin used to work this way too; since 2026-07-29 it reads its newest row instead, because that routine no longer commits.)
