...
- **THE RUN NOTE IS BEHIND A `Run notes` DISCLOSURE, AFTER THE CANDIDATES** (2026-08-18). It used to be glued onto the end of the dialog's head line, which is one sentence about twice a week and two hundred words the rest of the time. At that length it filled the panel on a laptop, pushed all fifteen candidates below the fold, and left **OPEN ROUTINE** and **CLOSE** as the only controls on screen, so the WAYPOINT BOT button read as a link to the routine and nothing else. The head line now carries the city and the two counts only. Same order rule every card in this project follows: what it is, then what you can do about it, then the small print.
