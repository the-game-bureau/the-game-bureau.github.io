...
**TGB WAYPOINT BOT IS RETIRED AS OF 2026-08-20 AND ITS TRIGGER MUST BE SWITCHED OFF BY HAND.** It was folded into TGB PATH BOT the same day it was rebuilt; see the merge note in the TGB PATH BOT section below for what moved and what was given up. **The trigger `trig_018FbHnaU5DqB4GesPfABV2d` was still enabled when this was written**, because a Claude Code REMOTE session cannot edit a routine created through the website (`update_trigger` answers *this routine was created via http_api, not by an agent*), so neither disabling the old one nor repasting the new prompt could be done from the commit that made every other change here. Until somebody disables it, it runs twice a day and commits `mc/stops/nightly.json`, a file this repo no longer contains and nothing reads. Do it at claude.ai, or from a local Claude Code with `/schedule`.
