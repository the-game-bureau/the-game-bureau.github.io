...
--   * HOMETOWN is derived, not typed. A guide is the AWAY fanbase's character,
--     and 23 of the 34 front games with exactly one away city between them -
--     Sir Purr appears only on Carolina games, Remy only on New Orleans ones -
--     so that city is the character's home and it is seeded here. All 16
--     distinct values already existed in public.cities, which is what makes
--     the foreign key safe to add validated rather than NOT VALID.
--
--     The other 11 are left null rather than guessed. Mission Control is the
--     placeholder that fronts 296 games across every away city in the
--     catalogue, so it has no hometown by definition; the remaining ten front
--     a single game each with no away club recorded. A wrong hometown reads
--     exactly like a right one, so the blank is the honest answer.
--
...
create table if not exists public.guides (
  ...
  image_url   text,
  -- WHERE THE CHARACTER IS FROM, as the canonical city string, and a real
  -- foreign key into the one city catalogue rather than free text. A guide is
  -- a fanbase's character - Sir Purr is Charlotte's, Remy is New Orleans' -
  -- and that is worth being able to query and group by, which "Charlotte" and
  -- "charlotte " and "Charlotte, NC" in a text box never allow.
  --
  -- It points at cities.city, not cities.slug, because the whole product
  -- already passes the canonical string around: games.city, waypoints.city,
  -- anchor_events.city and every TgbCities picker speak it, so a slug here
  -- would need translating at every boundary. on update cascade so correcting
  -- a city's canonical spelling carries; on delete set null so removing a city
  -- from the catalogue never takes a guide with it.
  hometown    text references public.cities(city) on update cascade on delete set null,
  archived    boolean not null default false,
  created_at  timestamptz not null default now(),
  updated_at  timestamptz not null default now()
);

create index if not exists guides_hometown_idx on public.guides (hometown);
...
insert into public.guides (name, bio, background, image_url, hometown)
values
  ('AL', 'Your guide is ready to rally Las Vegas fans before kickoff.', null, null, 'Las Vegas, Nevada'),
  ('Alan Smithee', 'This game is directed by Alan Smithee, famed director of such classics such as: Maniac Cop III: Badge of Silence and The Shrimp on the Barbie', null, null, null),
  ('Big Red', 'Your guide is ready to rally Arizona fans before kickoff.', null, null, 'Phoenix, Arizona'),
  ('Bucco Bruce', 'Arrrgh! I''ll be your guide for our takeover of New Orleans.', null, null, 'Tampa, Florida'),
...
Sample first-stop line: “Welcome to Pittsburgh. Feel how the city stacks itself up around you, river, bridge, brick, hill, all daring you to blink? Don’t. Plant your feet, lift your eyes, and let’s find the first seam in all this steel.”', null, 'Atlanta, Georgia'),
  ('Catherine', null, null, null, null),
  ('David Hennessey', null, null, null, null),
  ('David Hennessy', null, null, null, null),
  ('Freddie', 'Your guide is ready to rally Atlanta fans before kickoff.', null, null, 'Atlanta, Georgia'),
...
Sample first-stop line, at Puerta del Sol: "Stand right there on the kilometer-zero plate, because every road in Spain starts under your feet, which means the takeover starts under your feet too. The Bengals think this is their square today. Cute. Look up, find me, and let''s go take it one stripe at a time."', null, 'Atlanta, Georgia'),
  ('Gumbo', 'Gumbo has been the Saints mascot since their inception on November 1st 1966. He''s a SAINT Bernard. Get it? If you don''t get it, you''re probably going to have a tough time with our game. Anyway, he''s a majic talking dog. Get over it.', null, null, 'New Orleans, Louisiana'),
...
Sample first-stop line: Ground unit, Cleveland contact confirmed. You are standing in Browns territory, hostile noise on all sides, lake wind trying to scramble the signal. Hold position, confirm your visual, and advance on my mark.', null, 'Houston, Texas'),
...
Sample first-stop line: “Welcome to Chicago, baby. Feel that wind trying to boss you around? Let it talk. You and me are about to read this town from the sidewalk up, and by the time we’re done, these streets will know exactly who came calling.”', 'https://thegamebureau.com/mc/assets/guides/chc2026atl.png', 'Atlanta, Georgia'),
...
Sample first-stop line, at the bear statue on Puerta del Sol: "See that bear stretching for the little tree? That is Madrid''s tough guy, frozen mid-reach for two hundred years, which is what the ground does to anything that quits hunting. We did not fly across an ocean to pose for postcards. Put your boot on the kilometer-zero plate, because every road in Spain starts right there under you, and today every last one of them runs orange. Quiet feet. Who dey."', null, null),
  ('Nigel', 'Nigel is your longtime tour manager. Unfortunately he''s quitting to sell Lucky Gogs on Bourbon Street. Track him down and convince him to be your manager again.', null, null, null),
  ('Oliver', null, null, null, null),
  ('Packy', 'Your guide is ready to rally Green Bay fans before kickoff.', null, null, 'Green Bay, Wisconsin'),
...
Sample first-stop line: "Welcome to Kansas City, kid — capital of barbecue, fountains, and a whole lot of people wearing MY color and getting it wrong. Don''t let the Sea of Red fool you; there''s only one true redbird in this town, and he''s perched right above you. Look up. Good. Now look alive — the takeover starts here, and I''ve been waiting about a hundred years for this. Wings up."', null, 'Phoenix, Arizona'),
...
Sample first-stop line: All right, Boots, Decatur Street. See the golden girl on the horse, shining like she carries her own sunrise? I have perched on that crown more times than the home crowd would forgive, and here is what they never notice from the ground: she is not waving that flag, she is aiming it. Follow the line from the tip and tell me what it touches. And step quick, the air down there is thicker than my patience. Wings up, dust off.', null, 'Phoenix, Arizona'),
  ('Scrappy', null, null, null, null),
  ('Seymour', null, null, null, null),
  ('Sir Purr', 'Your guide is ready to text back and forth with you as you traverse the CBD and maybe even skirt the Quarter.', null, null, 'Charlotte, North Carolina'),
  ('SouthPaw', null, null, null, null),
...
Sample first-stop line: Welcome to Philadelphia. I rolled in ahead of you and counted three birds, two bad angles, and one city that thinks noise can stop Pittsburgh. Grab the handle, read what is in front of you, and keep it rolling.', null, 'Pittsburgh, Pennsylvania'),
  ('The Wagonmaster', 'A lifelong Bills Mafia road warrior who''s followed the team into every hostile stadium in the league. He''s not beside you, he''s in your ear and on your screen, calling the plays from somewhere out there while you stand on the real Denver pavement. That''s the trick: you''re physically here, but he''s running the show, turning the city''s actual streets and statues into your field of play. Circle the wagons and follow his lead.', null, null, 'Buffalo, New York'),
  ('Viktor', 'Of course we would never voice our game with a character copyrighted by a multi-billion dollar corporation, but if we woould, your game guide would be Viktor.', null, null, 'Minneapolis, Minnesota'),
...
Sample first-stop line: Baltimore crew, you are on Cincinnati pavement now. The home side has the address, the noise, and the easy assumptions. Good. Let them keep all three. You watch the signal, hold the line, and move when I clear the track.', null, 'Baltimore, Maryland')
