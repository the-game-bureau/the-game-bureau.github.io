...
begin
  ...
  --
  -- SECONDS, not minutes. To the minute, two imports of the same city and the
  -- same shape inside one minute produce the SAME id - so the second tour does
  -- not fail, it silently merges into the first and you get one twenty-stop
  -- walk. That is exactly what a re-paste after a typo looks like. now() is the
  -- statement timestamp, so every row of one call still shares one id, which is
  ...
  -- title and id ride on the note of the FIRST WAYPOINT instead, which is where
  -- a reader looks anyway: walk_order 1 is the start of the walk, so the walk's
  -- name belongs on it. Every row this function returns is now a real waypoint
  -- row, and every one of them reports action = 'waypoint'.
  ...
  loop
    ...
    -- 'waypoint', not 'stop'. In the product's own vocabulary a STOP is a game
    -- plus a waypoint plus a challenge, and it lives in public.stops. What this
    -- function inserts is a row in public.waypoints - a place - so calling the
    -- result a stop named the wrong one of the two tables.
