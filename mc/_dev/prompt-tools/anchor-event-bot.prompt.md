...
**THIS ROUTINE IS TGB ANCHOR BOT AND TGB CONCERT BOT, COMBINED (2026-08-28).**
They filed into the same table by two doorways on two schedules, and the second
was the narrower of the two: concerts only, ten a call, through a function whose
payload keys are a legacy contract rather than the column names. Everything it
knew that this file did not is folded in below, chiefly the SeatGeek sourcing
and the two rules about spread and lead time. **`tgb_pull_concert_tours` is not
used any more** and nothing should call it.
...
- **CHECK IT, DO NOT ASSUME IT.** Every NFL, NBA, NHL, MLB and major-college
  ...
  minor-league parks do not. **When you are not sure, look the venue up**, since its
...
**Read public pages the way a person would. No API keys**, because a cloud routine has
...
### Concerts, which used to be a routine of their own

- **PREFER A TOUR'S OWN DATE LIST** over a single event page. One tour gives you
  several cities, and several cities is exactly what this table wants: a game is
  built per city, so a second date in a city already filed adds almost nothing.
- **SPREAD THEM ACROSS CITIES AND ACTS.** Ten dates of one tour in one week is a
  worse run than ten different cities.
- **PREFER DATES FAR ENOUGH OUT TO BUILD FOR.** A concert three days from now
  ...
  range; inside three weeks, take it only if it is genuinely notable.
- **The 10,000 seat rule applies to a concert exactly as it does to a fixture.**
  A club show is not an anchor event, however good the act.
...
**THE KEYS ARE THE COLUMN NAMES**, so what you send is what a human sees on the
page. That was the reason to keep this doorway and retire the concert one, whose
payload keys were a legacy contract that had to be mapped.
...
- the spread: how many sports, concerts, conventions, and across how many
  cities. **Concerts are now this routine's job as well**, so a run with none at
  all is worth a sentence saying why;
