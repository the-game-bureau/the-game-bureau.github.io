...
/* â”€â”€ DETERMINISTIC MATH & HASHING â”€â”€ */
...
async function fetchConfiguredTeamNames(gameId) {
  const targetId = String(gameId || '').trim();
  if (!targetId || !SB_CONFIG.enabled) return [];

  try {
    const requestUrl = new URL(`rest/v1/${SB_CONFIG.table}`, `${SB_CONFIG.url.replace(/\/+$/, '')}/`);
    requestUrl.searchParams.set('select', 'nodes');
    ...
    return normalizeTeamNames(gameNode && gameNode.teams);
  } catch (error) {
    return [];
  }
}
...
const configuredTeamNamesPromise = fetchConfiguredTeamNames(GAME_ID).then((names) => {
  configuredTeamNames = normalizeTeamNames(names);
...
const App = (() => {
  ...
  const setupTeamBuilder = (count) => {
    ...
    let teamNames = getTeamNames(count);
